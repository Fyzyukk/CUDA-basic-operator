...
__global__ void softmax_gpu_v0(float* x, float* y, int N) {

    int tid = threadIdx.x;
    int gid = blockIdx.x * blockDim.x + threadIdx.x;

    // 加载数据到shared memory
    __shared__ float smem[1024]; // 一个block有1024个thread
    smem[tid] = x[gid];
    __syncthreads();
...
int main() {
    ...
    int blockSize = 1024;                            // 1024个thread(种类)
    ...
    softmax_gpu_v0<<<gridSize, blockSize>>>(x_device, y_device, N);
