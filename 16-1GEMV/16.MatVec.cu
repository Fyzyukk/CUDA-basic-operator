...
void gemv_kernel(T* vec, T* d_vec, T* mat, T* d_mat, T* dst, T* d_dst) {
    ...
    constexpr int VECS_PER_THREAD = (N / VEC_SIZE) / THREAD_NUMS; // N / VEC_SIZE: 处理多少次向量,
                                                                // / THREAD_NUMS: 每个线程处理多少次向量
    DispatchLauncher<VECS_PER_THREAD, VEC_SIZE, THREAD_NUMS>::template launcher<T>(d_mat,
                                                                        d_vec, d_dst, M, N);
